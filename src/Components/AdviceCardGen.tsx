...
import { Download, Share2, Check, Copy, Sparkles, Award, Star, HelpCircle, Twitter, Linkedin, Send } from 'lucide-react';
...
export default function AdviceCardGen({ initialCard }: AdviceCardGenProps) {
  ...
  const [larkSharing, setLarkSharing] = useState<'idle' | 'sending' | 'sent' | 'error'>('idle');
...
#Productivity #Business #Mentorship #DecisionOS`;
  ...
  const handleShareToLark = async () => {
    if (!card || larkSharing === 'sending') return;
    setLarkSharing('sending');
    try {
      const response = await fetch('/api/lark/share', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          headline: card.headline,
          coreFramework: card.coreFramework,
          steps: card.steps,
      ...
      setLarkSharing('sent');
      setTimeout(() => setLarkSharing('idle'), 3000);
    } catch {
      setLarkSharing('error');
      setTimeout(() => setLarkSharing('idle'), 3000);
    }
  };
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2.5 pt-1 border-t border-immersive-border/40">
          ...
          {/* Share to Lark Channel */}
          <button
            onClick={handleShareToLark}
            disabled={larkSharing === 'sending'}
            className={`w-full font-sans font-bold text-xs tracking-tight p-3 rounded-xl flex items-center justify-center gap-2 transition-all duration-200 border cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed ${
              larkSharing === 'sent'
                ? 'bg-emerald-500/15 border-emerald-500/40 text-emerald-400'
                : larkSharing === 'error'
                ? 'bg-rose-500/15 border-rose-500/40 text-rose-400'
                : 'bg-[#00D6B4]/10 hover:bg-[#00D6B4]/15 text-[#00D6B4] border-[#00D6B4]/25'
            }`}
          >
            {larkSharing === 'sending' && (
              <><div className="animate-spin rounded-full h-3.5 w-3.5 border-2 border-[#00D6B4]/40 border-t-[#00D6B4]" /> Sending to Lark...</>
            )}
            {larkSharing === 'sent' && (
              <><Check className="h-3.5 w-3.5" /> Shared to Lark channel!</>
            )}
            {larkSharing === 'error' && (
              <><Share2 className="h-3.5 w-3.5" /> Lark not configured (set LARK_WEBHOOK_URL)</>
            )}
            {larkSharing === 'idle' && (
              <><Send className="h-3.5 w-3.5" /> Share to Lark Channel</>
            )}
          </button>
